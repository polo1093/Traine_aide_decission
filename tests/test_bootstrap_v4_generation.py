...
from experiments.generate_bootstrap_v4 import (
    build_v4_plan,
...
def test_v4_plan_uses_exact_staged_solve_count_and_hero_contexts() -> None:
    plan = build_v4_plan(target_solves=50, seed=9400)
    ...
    assert all(row["scenario"].startswith("v4_") for row in plan)


def test_v4_report_flags_root_player_not_hero_as_critical() -> None:
    records = [
...
def test_v4_report_accepts_balanced_expected_bootstrap_warnings() -> None:
    export_summary = {
        "status": "ok",
